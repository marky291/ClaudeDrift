...
1. **`scripts/discover.mjs`** (Node, zero dependencies) enumerates every artifact
   ...
   the live filesystem and package manifests. It applies precision filters so it
   doesn't flag globs, placeholders, external `~/` paths, runtime-generated files,
   example names, or paths a doc explicitly describes as removed; every suppressed
   candidate is reported with a reason. A case-sensitive existence check catches
   refs that work on macOS but break on Linux/CI. Node is always present because
   Claude Code itself runs on Node.
